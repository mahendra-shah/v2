 ...
 * Made web-llm import dynamic to prevent build errors
 */
export const useLLM = () => {
  ...
  const [webllmModule, setWebllmModule] = useState(null)

  const engineRef = useRef(null)

  // Check WebGPU support and load module on mount
  useEffect(() => {
    const checkWebGPUAndLoadModule = async () => {
      try {
        ...
        try {
          ...
          setWebllmModule(webllm)
          setIsWebGPUSupported(true)
        } catch (importError) {
          console.warn("Failed to load web-llm module:", importError)
          setIsWebGPUSupported(false)
          setError("AI module failed to load. Chat will use fallback mode.")
        }
      } catch (err) {
  ...
  const initializeModel = useCallback(async () => {
    if (!isWebGPUSupported || !webllmModule) {
      return false
    }
  ...
  }, [isWebGPUSupported, webllmModule])
